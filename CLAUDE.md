...
1. `POST /` → `src/routes/+page.server.ts` — rate-limit (CF Rate Limiter binding), Turnstile verification, insert into `timestamps` table, redirect to `/:id`
2. `GET /:id` → `src/routes/[id]/+page.server.ts` — fetch row by UUID, pass `ts` (UTC ISO) + `creatorTimezone` to page
3. `src/routes/[id]/+page.svelte` — `onMount` detects viewer timezone, formats both times client-side. Uses `{#if mounted}` guard to avoid SSR hydration mismatch.
